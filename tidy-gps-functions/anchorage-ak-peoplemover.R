...
tidy_gps_people_mover <- function() {
  ...
  tidy_gps <- data.frame(lat = bus_latitude, lon = bus_longitude, route = bus_route) %>% filter(!bus$
  ...
  tidy_gps %>% filter(!is.na(direction)) %>% mutate(datetime = Sys.time(), lat = as.numeric(as.chara$
}
